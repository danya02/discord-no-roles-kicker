...
pub async fn run(ctx: &Context, command: &ApplicationCommandInteraction) -> Result<(), String> {
    ...
    let rule = match sqlx::query!("SELECT * FROM guild_rule WHERE guild_id = ?", guild_id).fetch_one(pool).await {
        Ok(rule) => rule,
        Err(why) => return Err(format!("Error while getting current active rule (did you `/setup`?): {why}")),
    };

    let mut text = vec!();
    text.push(format!("Current rule:"));
    text.push(format!("System message channel: <#{}> `/syschannel`", rule.system_message_channel_id));
    if let Some(c) = rule.pending_kick_notification_channel_id {
        text.push(format!("Will remind about pending kicks in: <#{}> `/pendingchannel`", c));
    } else {
        text.push(format!("Will remind about pending kicks in: unset `/pendingchannel`"));
    }
    text.push(format!("Reminders will be sent at seconds before kick: {:?} `/pendingreminders`", rule.pending_kick_notification_values));

    if let Some(t) = rule.new_member_kick_timeout{
        let dur = Duration::from_secs(t as u64);
        text.push(format!("New members get kicked after: {} seconds = {} `/newtimeout`", t, humantime::format_duration(dur)));
    } else {
    ...
    if let Some(t) = rule.new_member_kick_timeout{
        let dur = Duration::from_secs(t as u64);
        text.push(format!("Members losing immunity role get kicked after: {} seconds = {} `/immunitytimeout`", t, humantime::format_duration(dur)));
    } else {
        text.push(format!("Members losing immunity role get kicked after: never `/immunitytimeout`"));
    }

    let dur = Duration::from_secs(rule.kick_safety_timeout as u64);
    text.push(format!("When kicking a member, allow cancelling within: {} seconds = {} `/safetytimeout`", rule.kick_safety_timeout, humantime::format_duration(dur)));
    ...
    let res = command.create_interaction_response(&ctx.http, |resp|{
        ...
        .interaction_response_data(|msg| msg.content(text.join("\n")))
    }).await;
    if let Err(why) = res {
        error!("Error while responding to rule interaction: {why}");
